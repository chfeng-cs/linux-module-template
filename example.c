...
static void vma_list(struct task_struct *ts)
{
	...
	for(vma = mm->mmap; vma; vma = vma->vm_next)
	{
        ...
        if (vma->vm_flags & VM_WRITE)
        	...
        	my_flags[2] = '-';
        printk("%2d: %s\t0x%012lx\t0x%012lx\t%4ld KiB", count, my_flags, vma->vm_start, vma->vm_end, (vma->vm_end-vma->vm_start)/1024);
        // printk("%p %p\n", vma->vm_prev, vma->vm_next);
	}
...
static ssize_t mtest_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	else if (memcmp(buf, "info", 4) == 0) {
		struct task_struct *ts = NULL;
        pid_t pid;
        if (sscanf(buf+4, " %d", &pid) > 0) {
            ts = pid_task(make(pid), PIDTYPE_PID);
            if (ts != NULL) {
                printk("cpu: %u\n", ts->cpu);
                printk("comm: %s\n", ts->comm);
                vma_list(ts);
